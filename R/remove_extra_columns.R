...
clean_data.tool1_kdr_for_client <- clean_data.tool1_kdr
...
# Tool 1 KDR
clean_data.tool1_kdr_for_client$data <- clean_data.tool1_kdr$data %>% select(-any_of(c(extra_cols.tool1_kdr, qa_cols)))

for (sheet in names(clean_data.tool1_kdr_for_client)[-1]){
  clean_data.tool1_kdr_for_client[[sheet]] <- clean_data.tool1_kdr_for_client[[sheet]] |> select(-any_of(c(extra_cols.tool1_kdr, qa_cols, "starttime"))) |>
    select(Site_Visit_ID, EMIS_School_ID_CBE_KEY, everything())
}
